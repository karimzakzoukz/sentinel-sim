...
async def pay(amount: int = 100) -> dict[str, str]:
    """Fake payment endpoint — returns a fee calculated as amount/amount.
    Handles zero amount gracefully by returning a fee of 0.
    """
    log.info("Processing payment amount=%s", amount)
    if amount == 0:
        # Avoid division by zero; define fee as 0 for zero amount
        fee = 0
    else:
        fee = amount / amount
